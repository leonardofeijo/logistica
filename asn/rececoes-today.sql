...
SELECT DISTINCT REC.CODE, REC.FORN, REC.COMPONENT, SUM(REC.QTF) AS QTF, REC.DLVR AS DLVR, 
(SELECT MAX(ET0192) FROM AUTO.D805DATHPO.FET001) AS TODAY
    FROM (
        SELECT ET0111 AS FORN, ET0110 AS CODE, ET0102 AS COMPONENT, FLOAT(ET0103) AS QTF, INT(FET.ET0114) AS DLVR
            FROM AUTO.D805DATHPO.FET001 FET
        WHERE ET0114 > '20240625' AND ET0110 <> ''
    ) REC  
GROUP BY COMPONENT, CODE, DLVR, FORN
ORDER BY COMPONENT

--

SELECT CODE, FORN, COMPONENT, QTF, TODAY
    FROM (
        SELECT DISTINCT REC.CODE, REC.FORN, REC.COMPONENT, SUM(REC.QTF) AS QTF, REC.DLVR AS DLVR, 
        (SELECT MAX(ET0192) FROM AUTO.D805DATHPO.FET001) AS TODAY
            FROM (
...
WHERE MPPO.TODAY = MPPO.DLVR
ORDER BY CODE

--

-- BUSCAR AVIES ALTERADO PARA 4 NO DIA ATUAL
